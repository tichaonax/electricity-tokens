...
class ServiceUninstaller {
  ...
  async checkServiceExists() {
    ...
    try {
      ...
      const result = execSync(
        `${config.commands.SC_COMMAND} query "${config.buildServiceExpectedName(config.name)}"`,
        {
          encoding: 'utf8',
        }
      );
  ...
  async stopService() {
    ...
    try {
      const { execSync } = require('child_process');
      execSync(
        `${config.commands.SC_COMMAND} stop "${config.buildServiceExpectedName(config.name)}"`,
        {
          stdio: 'pipe',
        }
      );
...
async function main() {
  ...
  try {
    ...
    console.error(
      `  ${config.commands.SC_COMMAND} delete "${config.buildServiceExpectedName(config.name)}"`
    );
