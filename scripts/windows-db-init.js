...
async function killPrismaProcesses() {
  ...
  try {
    ...
    await execAsync(
      'taskkill /F /IM prisma.exe 2>nul || echo No prisma.exe processes found'
    );
    await execAsync(
      'taskkill /F /IM node.exe /FI "WINDOWTITLE eq *prisma*" 2>nul || echo No prisma node processes found'
    );
...
async function clearPrismaCache() {
  ...
  try {
    ...
    if (fs.existsSync(prismaDir)) {
      await execAsync(
        `rmdir /s /q "${prismaDir}" 2>nul || echo Cache already clean`
      );
...
async function initializeDatabaseWindows() {
  console.log(
    '🚀 Initializing database for Electricity Tokens Tracker (Windows)...'
  );
  console.log('');
  ...
  try {
    ...
    console.log(
      `📊 Current counts: Users(${userCount}), Purchases(${purchaseCount}), Contributions(${contributionCount})`
    );
    console.log('');
    ...
    console.log(
      '4. Promote user to admin if needed: node scripts/create-admin.js'
    );
