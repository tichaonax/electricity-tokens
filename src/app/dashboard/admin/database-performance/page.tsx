...
import { Database, CheckCircle, AlertTriangle, RefreshCw, Zap, Clock, TrendingUp } from 'lucide-react';
        ...
        <div className="px-4 py-6 sm:px-0">
          <div className="mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-300">
              Monitor and optimize your database performance by creating indexes and running performance checks.
            </p>
          </div>

          {message && (
            <Alert className={`mb-6 ${message.type === 'success' ? 'border-green-200 bg-green-50 dark:bg-green-950' : 'border-red-200 bg-red-50 dark:bg-red-950'}`}>
              ...
              <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium">{stats.avgQueryTime.toFixed(2)}ms</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Index Hit Ratio</span>
                    <span className="text-sm font-medium">{stats.indexHitRatio.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Slow Queries</span>
                    <span className="text-sm font-medium">{stats.slowQueries}</span>
                  </div>
                  {stats.lastOptimized && (
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600 dark:text-gray-400">Last Optimized</span>
                      <span className="text-sm font-medium">{new Date(stats.lastOptimized).toLocaleDateString()}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <CardContent>
              {indexes.length === 0 ? (
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  Click "Check Status" to view database index information
                </div>
              ) : (
                  ...
                  {indexes.map((index, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 text-xs rounded-full ${getImpactBadge(index.estimatedImpact)}`}>
                          {index.estimatedImpact} impact
                        </span>
                        <span className={`text-sm font-medium ${index.exists ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'}`}>
          ...
          <Alert className="mt-6 border-yellow-200 bg-yellow-50 dark:bg-yellow-950">
            ...
            <AlertDescription className="text-yellow-800 dark:text-yellow-200">
              <strong>Important:</strong> Database optimization should be run during low-traffic periods. 
              Creating indexes can temporarily affect performance during the operation.
            </AlertDescription>
          </Alert>
        </div>
      </main>
